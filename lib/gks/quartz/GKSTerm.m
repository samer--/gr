...
static void handle_resize_draw(GKSTerm *gksterm, void *socket, unsigned char *data) {
  // Send acknowledgement before actually drawing to avoid timeout
  char reply[1];
  reply[0] = GKSTERM_FUNCTION_RESIZE_DRAW;
  send_message(socket, reply, sizeof(reply));

  int window = *(int*)data;       data += sizeof(int);
  double width = *(double*)data;  data += sizeof(double);
  double height = *(double*)data; data += sizeof(double);
  size_t displaylist_len = *(size_t*)data; data += sizeof(size_t);
  void *displaylist = (void*)data;
  dispatch_sync(dispatch_get_main_queue(), ^{
    NSData *displaylist_objc = [NSData dataWithBytesNoCopy: displaylist length: displaylist_len freeWhenDone: NO];
    [gksterm GKSQuartzResizeDraw: window displayList: displaylist_objc : width : height];
  });
}
...
static void handle_message(GKSTerm *gksterm, void *socket) {
  ...
  switch(data[0]) {
    case GKSTERM_FUNCTION_CREATE_WINDOW: handle_create_window(gksterm, socket, data+1); break;
    case GKSTERM_FUNCTION_DRAW:          handle_draw(gksterm, socket, data+1);          break;
    case GKSTERM_FUNCTION_RESIZE_DRAW:   handle_resize_draw(gksterm, socket, data+1);   break;
    case GKSTERM_FUNCTION_IS_ALIVE:      handle_is_alive(gksterm, socket, data+1);      break;
    case GKSTERM_FUNCTION_CLOSE_WINDOW:  handle_close_window(gksterm, socket, data+1);  break;
    case GKSTERM_FUNCTION_IS_RUNNING:    handle_is_running(gksterm, socket, data+1);    break;
    default:                             handle_unknown(socket, data+1);                break;
  }
  zmq_msg_close(&message);
}
